    ...
    public function verify($token,$status)
    {
        ...
        if (isset($result->status)) {
            if ($result->status == 1) {
                ...
                \Cart::clear();
                return ['success'=>$result->transId.'پرداخت با موفقیت انجام شد. شماره تراکنش: '];
            } else {
                return ['error'=>$result->status.'پرداخت با خطا مواجه شد. شماره وضعیت: '];
            }
        } else {
            if ($status == 0) {
                return ['error'=>$status.'پرداخت با خطا مواجه شد. شماره وضعیت: '];
            }
        }
    }
